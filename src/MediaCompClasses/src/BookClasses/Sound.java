...
public class Sound extends SimpleSound
{
  ...
  public static void main(String[] args)
  {
    ...
    public void writeSoundToFile(String path, String filename, Sound newSound) {
 ...
 public Sound fadeIn(int seconds) {
      /**
       * Method to get half of the sound
       * @author: Daniel & Julia
       * @ return: Sound
       */
        Sound mySound = new Sound(this.getLength());

      if (seconds > this.getLength()/this.getSamplingRate()) {
          System.out.println("Error: Seconds longer than file length");
      }
      else {
        double interval = seconds*this.getSamplingRate();
        double fade = 1 / interval;
        double fadeTwo = 1 / interval;
        for(int i =1; i < this.getLength(); i++) {
        if (i<interval) {
            int currentSample = this.getSampleValueAt(i);
            double currentSample2 = currentSample*fade;
            mySound.setSampleValueAt(i, (int)currentSample2);
            fade=fade+fadeTwo;
        }
        else {
            int currentSample = this.getSampleValueAt(i);
            mySound.setSampleValueAt(i, currentSample); 
        } 
        }  
    }
            return mySound;
